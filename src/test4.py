...
def handle_edit(message: Message, info):
    delete_message(409073849905315847, int(message.id)).then(handle_response).catch(catch_err)

def handle_message(message: Message, info):
    edit_message(409073849905315847, int(message.id), {"content": "hej"}).then(handle_edit).catch(catch_err)

get_channel(574383922167808003).then(handle_response).catch(catch_err)
create_message(409073849905315847, {"content": "hejsan", "nonce": int(Snowflake.from_date(datetime.now()))}).then(handle_message).catch(catch_err)
